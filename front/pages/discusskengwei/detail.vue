...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusskengweiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusskengweiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusskengwei');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusskengwei');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusskengwei', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusskengwei', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusskengwei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusskengwei', this.detail);
				this.$utils.msg('审核成功');
