<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="坑位编号" prop="kengweibianhao">
          <el-input v-model="ruleForm.kengweibianhao" 
              placeholder="坑位编号" clearable  :readonly="ro.kengweibianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="坑位编号" prop="kengweibianhao">
              <el-input v-model="ruleForm.kengweibianhao" 
                placeholder="坑位编号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="坑位状态" prop="kengweizhuangtai">
          <el-select :disabled="ro.kengweizhuangtai" v-model="ruleForm.kengweizhuangtai" placeholder="请选择坑位状态">
            <el-option
                v-for="(item,index) in kengweizhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="坑位状态" prop="kengweizhuangtai">
	      <el-input v-model="ruleForm.kengweizhuangtai"
                placeholder="坑位状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="公厕名称" prop="gongcemingcheng">
          <el-select :disabled="ro.gongcemingcheng" @change="gongcemingchengChange" v-model="ruleForm.gongcemingcheng" placeholder="请选择公厕名称">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.gongcemingcheng" label="公厕名称" prop="gongcemingcheng">
...
export default {
  ...
  methods: {
    ...
    // 下二随
    gongcemingchengChange () {
      this.$http({
        url: `follow/gongcexinxi/gongcemingcheng?columnValue=`+ this.ruleForm.gongcemingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.jutiweizhi){
                this.ruleForm.jutiweizhi = data.data.jutiweizhi
              }
              if(data.data.yuangonggonghao){
                this.ruleForm.yuangonggonghao = data.data.yuangonggonghao
              }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kengwei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kengweiCrossAddOrUpdateFlag = false;
						 this.parent.search();
