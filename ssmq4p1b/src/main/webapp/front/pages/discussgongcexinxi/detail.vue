...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussgongcexinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussgongcexinxiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussgongcexinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussgongcexinxi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussgongcexinxi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussgongcexinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussgongcexinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussgongcexinxi', this.detail);
				this.$utils.msg('审核成功');
